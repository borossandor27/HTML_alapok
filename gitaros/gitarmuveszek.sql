...
--
-- Gép: 127.0.0.1
-- Létrehozás ideje: 2024. Sze 26. 22:16
...
--
-- Adatbázis: `gitarmuveszek`
--
CREATE DATABASE IF NOT EXISTS `gitarmuveszek` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE `gitarmuveszek`;
...
--
CREATE TABLE `minden` (
`zenekarNev` varchar(22)
,`muveszNev` varchar(24)
,`nemzetiseg` varchar(16)
,`stilusNev` varchar(10)
);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `minden`  AS SELECT `zenekar`.`zenekarNev` AS `zenekarNev`, `muvesz`.`muveszNev` AS `muveszNev`, `muvesz`.`nemzetiseg` AS `nemzetiseg`, `stilus`.`stilusNev` AS `stilusNev` FROM (((`zenekar` join `kapcsolo` on(`zenekar`.`id` = `kapcsolo`.`zenekarID`)) join `muvesz` on(`kapcsolo`.`muveszID` = `muvesz`.`id`)) join `stilus` on(`muvesz`.`stilusID` = `stilus`.`id`)) ORDER BY `zenekar`.`zenekarNev` ASC ;
